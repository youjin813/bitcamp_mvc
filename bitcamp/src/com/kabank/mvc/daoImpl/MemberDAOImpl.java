...
public class MemberDAOImpl implements MemberDAO{
	...
	public ResultMap login(Command cmd) {
		...
		System.out.println("ID :"+cmd.getMember().getPass());
		sql.replace(sql.indexOf("$"), sql.indexOf("$")+1, cmd.getMember().getId());
		sql.replace(sql.indexOf("@"), sql.indexOf("@")+1, cmd.getMember().getPass());
